...
public static class NodeExtensions
{
    public static TNode Instantiate<TNode>(this Node node, PackedScene model)
        where TNode : Node
    {
        TNode newNode = model.Instantiate<TNode>();
    ...
    {
        TNode2D newNode = model.Instantiate<TNode2D>();
        node.AddChild(newNode);
    ...
    public static void GoToScene(this Node node, string sceneName)
    {
        node.GetTree().ChangeSceneToFile(@"res://Scenes/" + sceneName + ".tscn");
    }
}
